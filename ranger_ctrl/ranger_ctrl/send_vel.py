...
import numpy as np
import transforms3d
from ranger_msgs.srv import NavigateToGoal
...
class VelocityPublisher(Node):
    def __init__(self):
        ...
        self.create_subscription (TFMessage, '/ranger_mini/tf', self.map_callback,10)
        ...
        self.srv = self.create_service (NavigateToGoal, 'navigate_to_goal', self.navigate_to_goal_callback)
    ...
    def navigate_to_goal_callback (self, request, response):
        print (f"navigating to goal {request.x}, {request.y}, {request.theta}")
        self.navigate_to_goal (request.x, request.y, request.theta)
...
#         self.create_subscription (TFMessage, 'ranger_mini/tf', self.tf_callback, 10)
